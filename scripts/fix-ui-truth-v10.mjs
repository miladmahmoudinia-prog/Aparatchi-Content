...
await patch('scripts/sync-upera.mjs', (source) => {
  ...
  const newSeek = `    let seekSeconds = 120;\n    try {\n      const probe = await execFileAsync(\n        'ffprobe',\n        ['-v', 'error', '-show_entries', 'format=duration', '-of', 'default=noprint_wrappers=1:nokey=1', source],\n        { timeout: Math.min(requestTimeoutMs, 12000), maxBuffer: 1024 * 1024 },\n      );\n      const duration = Number(String(probe?.stdout || '').trim());\n      if (Number.isFinite(duration) && duration > 20) seekSeconds = Math.max(10, Math.round(duration * 0.5));\n    } catch {\n      // Keep a safe interior fallback if duration probing is unavailable.\n      seekSeconds = 90 + ((Number(group?.episodeNumber || 1) * 17) % 90);\n    }\n\n    await execFileAsync(\n      'ffmpeg',\n      [\n        '-hide_banner',\n        '-loglevel', 'error',\n        '-ss', String(seekSeconds),\n        '-i', source,`;
  next = replaceOnce(next, oldSeek, newSeek, 'middle episode frame');
  return next;
});

await patch('.github/workflows/sync-upera.yml', (source) => {
  let next = source;
  next = replaceOnce(next, "          APARATCHI_RUN_TIME_LIMIT_MINUTES: '6'", "          APARATCHI_RUN_TIME_LIMIT_MINUTES: '12'", 'episode artwork time budget');
  next = replaceOnce(next, "          APARATCHI_EPISODE_ARTWORK_SERIES_PER_RUN: '24'", "          APARATCHI_EPISODE_ARTWORK_SERIES_PER_RUN: '120'", 'episode artwork series budget');
  next = replaceOnce(next, "          APARATCHI_EPISODE_ARTWORK_MIRROR_PER_RUN: '36'", "          APARATCHI_EPISODE_ARTWORK_MIRROR_PER_RUN: '0'", 'disable untrusted episode mirror');
  next = replaceOnce(next, "          APARATCHI_EPISODE_FRAME_CAPTURES_PER_RUN: '36'", "          APARATCHI_EPISODE_FRAME_CAPTURES_PER_RUN: '160'", 'episode frame capture budget');
  return next;
});
...
await patch('scripts/tests/mock-sync-fetch.mjs', (source) => {
  ...
  return replaceOnce(source, anchor, addition, 'movie detail truth test mock');
});
